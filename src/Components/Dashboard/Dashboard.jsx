...
const Dashboard = ({ currentUser, setCurrentUser }) => {
  ...
  useEffect(() => {
    fetch("https://crm-server-app.herokuapp.com/dashboard", {
      method: "GET",
      headers: {
      ...
      <Row>
        <Col md={4}>
          <Card border="primary">
            ...
            <Card.Body>
              <Card.Title>
                Total Service Request {state.service.length}
              </Card.Title>
              <Card.Text></Card.Text>
              <Button
                variant="primary"
        ...
        <Col>
          {currentUser.permission === "edit" ? (
            <Alert severity="success">
              <AlertTitle>Access</AlertTitle>Hello {currentUser.name} you can
              view, add and edit Service request, Leads and Contacts.
            </Alert>
          ) : currentUser.permission === "view" ? (
            <Alert severity="warning">
              <AlertTitle>Access</AlertTitle>Hello {currentUser.name} you can
              only view Service request, Leads and Contacts.
            </Alert>
          ) : (
            <Alert severity="warning">
              <AlertTitle>Access</AlertTitle>Hello {currentUser.name} you do not
              have access to view, add and edit Service request, Leads and
              Contacts.
            </Alert>
          )}
        </Col>
